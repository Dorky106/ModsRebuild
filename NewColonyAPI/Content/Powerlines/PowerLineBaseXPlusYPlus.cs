...
namespace PhentrixGames.NewColonyAPI.Content.Powerlines
{
    ...
    public class PowerLineBaseXPlusYPlus : Types.BaseType, IUpdatedAdjacentType, ISingleBlockEntityMapping
    {
        public PowerLineBaseXPlusYPlus() : base("PowerLineBaseXPlusYPlus", false)
        {
            if (MeshManager.GetMesh(NewColonyAPIEntry.ModName, "newpowerlinel", out string mesh))
            {
                this.Mesh = mesh;
            }
            this.MeshRotationEuler = new Pipliz.Vector3Int(0, 90, 270);
