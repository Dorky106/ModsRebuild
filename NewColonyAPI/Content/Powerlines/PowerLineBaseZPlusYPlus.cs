...
namespace PhentrixGames.NewColonyAPI.Content.Powerlines
{
    ...
    public class PowerLineBaseZPlusYPlus : Types.BaseType, IUpdatedAdjacentType, ISingleBlockEntityMapping
    {
        public PowerLineBaseZPlusYPlus() : base("PowerLineBaseZPlusYPlus", false)
        {
            if (MeshManager.GetMesh(NewColonyAPIEntry.ModName, "newpowerlinel", out string mesh))
            {
                this.Mesh = mesh;
            }
            this.MeshRotationEuler = new Pipliz.Vector3Int(0, 180, 270);
