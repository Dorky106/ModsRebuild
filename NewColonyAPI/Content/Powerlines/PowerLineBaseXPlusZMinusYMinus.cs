...
namespace PhentrixGames.NewColonyAPI.Content.Powerlines
{
    ...
    public class PowerLineBaseXPlusZMinusYMinus : Types.BaseType, IUpdatedAdjacentType, ISingleBlockEntityMapping
    {
        public PowerLineBaseXPlusZMinusYMinus() : base("PowerLineBaseXPlusZMinusYMinus", false)
        {
            if (MeshManager.GetMesh(NewColonyAPIEntry.ModName, "newpowerlinel1", out string mesh))
            {
                this.Mesh = mesh;
            }
            this.MeshRotationEuler = new Pipliz.Vector3Int(0, 270, 180);
