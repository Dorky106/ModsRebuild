...
namespace PhentrixGames.NewColonyAPI.Content.Powerlines
{
    ...
    public class PowerLineBaseXMinusZPlusYMinus : Types.BaseType, IUpdatedAdjacentType, ISingleBlockEntityMapping
    {
        public PowerLineBaseXMinusZPlusYMinus() : base("PowerLineBaseXMinusZPlusYMinus", false)
        {
            if (MeshManager.GetMesh(NewColonyAPIEntry.ModName, "newpowerlinel1", out string mesh))
            {
                this.Mesh = mesh;
            }
            this.MeshRotationEuler = new Pipliz.Vector3Int(0, 90, 180);
