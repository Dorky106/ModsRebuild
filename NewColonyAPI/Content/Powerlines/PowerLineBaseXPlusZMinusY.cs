...
namespace PhentrixGames.NewColonyAPI.Content.Powerlines
{
    ...
    public class PowerLineBaseXPlusZMinusY : Types.BaseType, IUpdatedAdjacentType, ISingleBlockEntityMapping
    {
        public PowerLineBaseXPlusZMinusY() : base("PowerLineBaseXPlusZMinusY", false)
        {
            if (MeshManager.GetMesh(NewColonyAPIEntry.ModName, "newpowerlinet1", out string mesh))
            {
                this.Mesh = mesh;
            }
            this.MeshRotationEuler = new Pipliz.Vector3Int(0, 180, 90);
